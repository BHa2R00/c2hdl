           ...
           (if (and
             ...
             (let ((pc (car line))
               ...
               (cond
                 ...
                 ((assoc (car asm) op :test #'equalp)
                  (let ((r0 (nth 1 asm))
                    ...
                    (cond
                      ...
                      ((char= #\i (char (reverse (car asm)) 0))
                       (format f "~A <= $signed(~A) ~A $signed(~A); " r0 r1 p i1))
                      (t (format f "~A <= $unsigned(~A) ~A $unsigned(~A); " r0 r1 p i1))
                      )
                    ))
                 ...
                 ((member (car asm) '("j" "b") :test #'equalp)
                  (let ((r0 (nth 1 asm)))
                    (format f "~A <= ~A; " *pc* r0)
                    ))
                 ((member (car asm) '("jalr" "jr") :test #'equalp)
                  (let ((r0 (nth 1 asm))
                        (r1 (nth 2 asm)))
                 ...
                 ((assoc (car asm) br3 :test #'equalp)
                  (let ((r0 (nth 1 asm))
                    ...
                    (format f "if($unsigned(~A) ~A $unsigned(~A)) ~A <= ~A; " r0 o r1 *pc* i1)
                    ))
                 ...
                 ((member (car asm) '("lw" "lwu") :test #'equalp)
                  (let ((rd (nth 1 asm))
                        ...
                        (r1 (nth 3 asm)))
                    (setf r1 (string-trim '(#\( #\)) r1))
                    (format f "if(!~A) begin ~A = $signed(~A) + $signed(~A); ~A <= 1; ~A <= 0; ~A <= 2; ~A <= ~A; end " *valid* *addr* of r1 *valid* *write* *size* *pc* *pc*)
