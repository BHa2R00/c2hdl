               ...
               (cond
                 ...
                 ((member (car asm) '("sw" "swu") :test #'equalp)
                  (let ((rd (nth 1 asm))
                        ...
                        (r1 (nth 3 asm)))
                    (setf r1 (string-trim '(#\( #\)) r1))
                    ;(format f "if(!~A && !~A) begin ~A = $signed(~A) + $signed(~A); ~A <= 1; ~A <= 1; ~A <= 2; ~A <= ~A; ~A <= ~A; end " *valid* *ready* *addr* of r1 *valid* *write* *size* *wdata* rd *pc* *pc*)
                 ...
                 ((member (car asm) '("lh" "lhu") :test #'equalp)
                  (let ((rd (nth 1 asm))
                        ...
                        (r1 (nth 3 asm)))
                    (setf r1 (string-trim '(#\( #\)) r1))
                    ;(format f "if(!~A && !~A) begin ~A = $signed(~A) + $signed(~A); ~A <= 1; ~A <= 0; ~A <= 1; ~A <= ~A; end " *valid* *ready* *addr* of r1 *valid* *write* *size* *pc* *pc*)
                 ...
                 ((member (car asm) '("sh" "shu") :test #'equalp)
                  (let ((rd (nth 1 asm))
                        ...
                        (r1 (nth 3 asm)))
                    (setf r1 (string-trim '(#\( #\)) r1))
                    ;(format f "if(!~A && !~A) begin ~A = $signed(~A) + $signed(~A); ~A <= 1; ~A <= 1; ~A <= 1; ~A <= ~A[15:0]<<(8*~A[1:0]); ~A <= ~A; end " *valid* *ready* *addr* of r1 *valid* *write* *size* *wdata* rd *addr* *pc* *pc*)
                 ...
                 ((member (car asm) '("lb" "lbu") :test #'equalp)
                  (let ((rd (nth 1 asm))
                        ...
                        (r1 (nth 3 asm)))
                    (setf r1 (string-trim '(#\( #\)) r1))
                    ;(format f "if(!~A && !~A) begin ~A = $signed(~A) + $signed(~A); ~A <= 1; ~A <= 0; ~A <= 0; ~A <= ~A; end " *valid* *ready* *addr* of r1 *valid* *write* *size* *pc* *pc*)
                 ...
                 ((member (car asm) '("sb" "sbu") :test #'equalp)
                  (let ((rd (nth 1 asm))
                        ...
                        (r1 (nth 3 asm)))
                    (setf r1 (string-trim '(#\( #\)) r1))
                    ;(format f "if(!~A && !~A) begin ~A = $signed(~A) + $signed(~A); ~A <= 1; ~A <= 1; ~A <= 0; ~A <= ~A[7:0]<<(8*~A[1:0]); ~A <= ~A; end " *valid* *ready* *addr* of r1 *valid* *write* *size* *wdata* rd *addr* *pc* *pc*)
